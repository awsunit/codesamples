...
// - false: if the iterator cannot be advanced (eg, it's "past
//   the end").  The iterator is no longer valid at this point.
bool LLIterator_Next(LLIterator *iter);

// Returns the payload of the list node that the iterator currently
// points at.  The iterator must be valid (eg, not "past the end").
//
// Arguments:
...
//   the tail.
// The iterator must be valid (eg, not "past the end").
//
// Arguments:
