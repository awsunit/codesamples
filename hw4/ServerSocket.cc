...
/*
  ...
  Calls getnameinfo on addr, retrieving information on success, 
  handling errors msgs.

  args: addr - the sockaddr in question
        caddr_len - size of addr
        hostname - return paramater char buffer
        hostlength - size of hostname
        servername/length - same as host, but for server
        flags - flags for getnameinfo
        errormsg - printed upon failure
  returns: true iff getnameinfo returns 0
           false otherwise

*/
static bool Accept_GNI(struct sockaddr *addr, socklen_t caddr_len,
                       char *hostname, socklen_t hostlength,
                       char *servername, socklen_t serverlength,
                       int flags, const string errormsg);
...
bool ServerSocket::BindAndListen(int ai_family, int *listen_fd) {
  ...
  int res = getaddrinfo(nullptr,
            (std::to_string(port_)).c_str(), &hints, &result);
  if (res != 0) {
  ...
  while (1) {
    ...
    if (addr->sa_family == AF_INET) {
      ...
      *client_port = ntohs(in4->sin_port);  // converted
      r &= Accept_GNI(addr, caddr_len, hostname, 1024, nullptr,
                      0, 0, "Problem resolving hostname");
      *client_dnsname = hostname;  // NOT CONVERTED
      // server information
      struct sockaddr_in *sin4 =
                          reinterpret_cast<struct sockaddr_in *>(&server);
      inet_ntop(AF_INET, &(sin4->sin_addr), astring, INET_ADDRSTRLEN);
      *server_addr = astring;  // converted
      r &= Accept_GNI(&server, server_len, hostname, 1024, nullptr,
                      0, 0, "problem resolivin server host name");
      ...
      *client_port = ntohs(in6->sin6_port);  // converted
      r &= Accept_GNI(addr, caddr_len, hostname, 1024, nullptr,
                      0, 0, "Problem resolving client host name");
      *client_dnsname = hostname;  // NOT CONVERTED
      // server information
      struct sockaddr_in6 *sin6 =
                           reinterpret_cast<struct sockaddr_in6 *>(&server);
      inet_ntop(AF_INET6, &(sin6->sin6_addr), astring, INET6_ADDRSTRLEN);
      *server_addr = astring;  // NOT CONVERTED
      r &= Accept_GNI(&server, server_len, hostname, 1024, nullptr,
                      0, 0, "Problem resolving server hostname");
      *server_dnsname = hostname;  // NOT CONVERTED
      *accepted_fd = client_fd;
      break;
    } else {
...
static bool Accept_GNI(struct sockaddr *addr, socklen_t caddr_len,
                       ...
                       char *servername, socklen_t serverlength,
                       int flags, const string errormsg) {
  if (getnameinfo(addr, caddr_len, hostname, hostlength,
                servername, serverlength, flags) != 0) {
